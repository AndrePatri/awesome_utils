...
IqOutRosGetter::IqOutRosGetter(double dt,
                         bool verbose,
                         double mov_avrg_cutoff_freq)
    :_dt{dt}, _verbose{verbose}, _mov_avrg_cutoff_freq{mov_avrg_cutoff_freq}
{
...
void IqOutRosGetter::set_jnt_names(std::vector<std::string> jnt_names)
{
...
void IqOutRosGetter::on_js_signal_received(const xbot_msgs::JointState& js_sig)
{
...
template <typename T, typename t_v >
int IqOutRosGetter::find_index(std::vector<T> input_v, t_v value)
{
    /**
    Finds the index of a value in an array.
